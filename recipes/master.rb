...
db_info = Hash.new
root_pw = String.new
chef_env = (node.chef_environment =~ /_default/ ? "production" : node.chef_environment)
...
search(:apps) do |app|
  ...
  (app['database_master_role'] & node.run_list.roles).each do |dbm_role|
    %w{ root repl debian }.each do |user|
      ...
      if !user_pw.nil? and user_pw[chef_env]
        Chef::Log.debug("Saving password for #{user} as node attribute node['mysql']['server_#{user}_password'")
        node.set['mysql']["server_#{user}_password"] = user_pw[chef_env]
        node.save
      else
        log "A password for MySQL user #{user} was not found in DataBag 'apps' item '#{app["id"]}' for environment ' for #{chef_env}'." do
          level :warn
        end
